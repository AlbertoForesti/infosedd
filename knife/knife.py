...
class KNIFE(nn.Module):
    ...
    def __call__(self, x, y):
        ...
        self.kernel_marg = MargKernel(self.args, self.y_dim, self.x_dim)
        self.kernel_cond = CondKernel(self.args, self.y_dim, self.x_dim)

        self.optimizer = torch.optim.Adam(self.parameters(), lr=self.args.lr)
        dataset = array_to_dataset(x, y)
        self.train_loader = torch.utils.data.DataLoader(dataset, batch_size=self.args.batch_size, shuffle=True)
        self.test_loader = torch.utils.data.DataLoader(dataset, batch_size=self.args.batch_size, shuffle=False)
        self.num_epochs = self.args.num_epochs
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.valid_interval = self.args.valid_interval
        self.to(self.device)
        ...
        for epoch in tqdm(range(self.num_epochs)):
            for batch in self.train_loader:
                ...
                mi, marg_ent, cond_ent = self.forward(batch)
                loss = marg_ent + cond_ent
                loss.backward()
                self.optimizer.step()
            if self.valid_interval > 0 and epoch % self.valid_interval == 0:
                ...
                for batch in self.test_loader:
                    with torch.no_grad():
                        batch = batch.to(self.device).float()
                        mi, marg_ent, cond_ent = self.forward(batch)
                    mis.append(mi.mean().detach().item())
                print(f"Epoch {epoch}: MI = {np.mean(mis)}")
        self.eval()
        mis = []
        for batch in self.test_loader:
            x = batch.clone()
    ...
    def forward(self, batch):  # samples have shape [sample_size, dim]
        x = batch.clone()
        y = batch.clone()
        x = x[:,:self.x_dim]
        y = y[:,self.x_dim:]
        marg_ent = self.kernel_marg(y)
        cond_ent = self.kernel_cond(x, y)
