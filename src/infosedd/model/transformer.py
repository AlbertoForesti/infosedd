...
#                                  Layers                                       #
#################################################################################
class LayerNorm(LightningModule):
...
class TimestepEmbedder(LightningModule):
    """
...
class LabelEmbedder(LightningModule):
    """
...
class DDiTBlock(LightningModule):
    ...
    def forward(self, x, c, seqlens=None):
        ...
        # Reshape and chunk the modulation output
        modulation = self.adaLN_modulation(c)  # shape: (batch_size, 6*dim)
        modulation = modulation.view(batch_size, 6, -1)  # shape: (batch_size, 6, dim)
        ...
        q, k, v = qkv.unbind(dim=2)  # Each has shape (b, s, h, d)
...
class DDitFinalLayer(LightningModule):
    ...
    def forward(self, x, c):
        # Reshape and chunk the modulation output
        modulation = self.adaLN_modulation(c)  # shape: (batch_size, 2*hidden_size)
        modulation = modulation.view(c.shape[0], 2, -1)  # shape: (batch_size, 2, hidden_size)
